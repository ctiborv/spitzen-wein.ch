<?php
//TODO: promyslet, zda bude vubec potreba a zda to vse nevyresi registration (nejakym rozsirenim)
abstract class Profile_View extends Template_Based_SpamProtected_Validated_View implements Mailing_View_Object
{
	protected $_do; // data object implementing the profile edit code
	...
	abstract protected function initDO(); // initializes the data object
	...
	protected function _updateFormTemplate()
	{
		...
		{
			$this->updateFromDO(); // load data object values into the form template
	...
	public function getMailBodyFields()
	{
		...
		foreach ($do_fields as $key => $value)
		{
			try
			{
				$fields[$key] = $this->getROFormattedValue($key, $value['property']);
			}
